...
/// A delete action button, typically using [FilledButton] as its base from Material 3.
///
/// Features:
...
class DeleteBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Set button color based on onPressed state
    effectiveStyle = effectiveStyle.copyWith(
      backgroundColor: WidgetStateProperty.resolveWith<Color?>((
        Set<WidgetState> states,
      ) {
        if (onPressed == null) {
          return Colors.grey; // Gray when onPressed is null
        }
        return Colors.red; // Red otherwise
      }),
    );
